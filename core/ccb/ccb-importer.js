...
function _createPrefabFromFile(ccbFile, targetPath, cb) {
    ...
    Async.waterfall([
        ...
        function(next) {
            // todo 检查这部分是从哪里调用的，这个文件隶属 core，但是 _Scene 是在 page 层内的
...
function _createAnimationClips(node, prefabPath, cb) {
    ...
    for (i = 0, n = sequences.length; i < n; i++) {
        ...
        timelines[i] = {
            ...
            loop: theInfo.chainedSequenceId === 0
        };
    ...
    for (i = 0, n = timelines.length; i < n; i++) {
        ...
        animClip.wrapMode = timeline.loop ? cc.WrapMode.Loop : cc.WrapMode.Normal;
...
function _parsePosition(data, theNodeObj, ret) {
    ...
    for (var i = 0, n = data.keyframes.length; i < n; i++) {
        ...
        var pos = _convertNodePos(theNodeObj, cc.v2(keyFrame.value[0], keyFrame.value[1]));
...
function _parseVisible(data, theNodeObj, ret) {
    ...
    for (var i = 0, n = data.keyframes.length; i < n; i++) {
        var keyFrame = data.keyframes[i];
        if (i === 0 && keyFrame.time > 0) {
            // the first frame is not at the beginning.
            // should add a in-visible frame at the beginning
            ret.props.active.push({
                frame: 0,
                value: false
            });
        }
...
function _createNodeGraph(rootNode, nodeData, curNodePath, parentSize, cb) {
    ...
    Async.waterfall([
        ...
        function(next) {
            ...
            Async.whilst(
                function(cb) {
                    cb(null, index < childrenData.length);
                },
                function(callback) {
...
function _initBaseProperties(node, props, parentSize) {
    ...
    node.angle = _getProperty(props, 'rotation', 0);
    ...
    node.color = new cc.Color(colorValue[0], colorValue[1], colorValue[2]);
    var opacityValue = _getProperty(props, 'opacity', 255);
    node.opacity = opacityValue;
}

function _getSpriteFrameUuid (frameData, defaultUrl) {
    var retUrl = '';
...
function _getSpriteFrame(frameData, defaultUrl) {
    let uuid = _getSpriteFrameUuid(frameData, defaultUrl);
    if (!uuid) {
        Editor.warn('Failed to import spriteframe asset, asset info: ' + frameData + ', defaultUrl: ' + defaultUrl);
        return null;
    }
...
function _initSprite(node, props, cb) {
    _initSpriteWithSizeMode(node, props, 'displayFrame', cc.Sprite.SizeMode.RAW, cb);
}

function _initSpriteWithSizeMode(node, props, frameKey, sizeMode, cb) {
    ...
    let uuid = _getSpriteFrameUuid(frameData, '');
    cc.assetManager.loadAny(uuid, function (err, spriteFrame) {
...
function _initScale9Sprite(node, props, cb) {
    Async.waterfall([
        function(next) {
            _initSpriteWithSizeMode(node, props, 'spriteFrame', cc.Sprite.SizeMode.CUSTOM, next);
        },
        function(next) {
...
function _initLabel(node, props, cb) {
    ...
    Async.waterfall([
        ...
        function(next) {
            ...
            if (fontSize[0] >= 0) {
                label._fontSize = fontSize[0];
                next();
            } else if (bmfntCfg) {
                cc.assetManager.loadAny({ url: Path.join(resTempPath, bmfntCfg) }, null, function(err, config) {
                    if (err) {
                        return next();
                    }
...
function _setFntFileForLabel(label, fntCfg, cb) {
    ...
    if (needLoadFnt) {
        cc.assetManager.loadAny(fntUuid, function(err, res) {
            if (err) {
                return cb();
            }
            label.font = res;
...
function _initControlButton(node, props, cb) {
    ...
    if (btnText) {
        ...
        _convertNodePos(labelNode, cc.v2(node.getContentSize().width / 2, node.getContentSize().height / 2));
...
function _initParticle(node, props, cb) {
    ...
    Editor.assetdb.queryMetaInfoByUuid(uuid, function (err, metaInfo) {
        if (err) {
            Editor.warn(err);
            return cb();
        }
        ...
        if (meta.type === 'raw') {
            Editor.info('The sprite component only supports the texture which imports as sprite type.');
            return cb();
        }
        else {
            ...
            cc.assetManager.loadAny(uuid, function (err, asset) {
                ...
                par.spriteFrame = asset;
                validPar = true;
                par.enabled = validPar;
                cb();
            });
        }
    });
